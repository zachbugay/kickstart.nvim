...
local mySysInfo = SystemInfo.new(vim.uv.os_uname())
...
require("lazy").setup({
  ...
  -- LSP Plugins
  {
    ...
    "folke/lazydev.nvim", -- Only load on lua files
    ft = "lua",
    opts = {
      library = {
  ...
  },
  {
    ...
    config = function()
      ...
      local function remove_x86_only_if_arm_processor(value)
        ...
        if mySysInfo.processor_architecture == "ARM" and (vim.tbl_contains(x86_only, value)) then
          return false
        end
        return true
      end
...
-- I want my custom options to override anything else.
